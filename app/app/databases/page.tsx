...
export default function DatabasesPage() {
  ...
  const submit = async () => {
    if (!title.trim() || !activeWorkspaceId) return toast.error("Add a name");
    setBusy(true);
    try {
  ...
  return (
    <PageContainer>
      <PageHeader title="Databases" subtitle="Structured tables for anything" icon={Data2} testId="databases-header"
        actions={<button onClick={() => setOpen(true)} className={btnPrimary} data-testid="new-database-btn"><Add variant="Bulk" size={18} /> New database</button>} />
      ...
      {dbs === undefined ? (
        ...
        <EmptyState icon={Data2} title="No databases yet" description="Create a Notion-style table to organize structured data." testId="databases-empty"
          action={<button onClick={() => setOpen(true)} className={btnPrimary}><Add variant="Bulk" size={18} /> New database</button>} />
      ) : (
          ...
          {dbs.map((d: any) => (
            <Link key={d._id} href={`/app/databases/${d._id}`} data-testid="database-card" className="group flex items-start gap-3 rounded-2xl border border-border bg-card p-4 transition-shadow hover:shadow-sm">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground">Updated {timeAgo(d.updatedAt)}</p>
              </div>
            </Link>
          ))}
        </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-sm" data-testid="database-create-dialog">
          ...
          <DialogFooter>
            ...
            <button onClick={submit} disabled={busy} className={btnPrimary} data-testid="database-create-submit">{busy ? "Creating\u2026" : "Create"}</button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </PageContainer>
