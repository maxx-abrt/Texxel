    ...
    <main className="min-h-screen overflow-x-hidden bg-background text-foreground">
      ...
      >
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-3.5 sm:px-6">
          ...
          <div className="hidden items-center gap-7 text-sm font-medium text-muted-foreground md:flex">
            ...
            <a href="#workspace" className="transition-colors hover:text-foreground">{t("navWorkspace")}</a>
            <a href="#connected" className="transition-colors hover:text-foreground">{t("navConnected")}</a>
          </div>
          ...
          <div className="tx-card overflow-hidden !rounded-[22px] elev-3">
            <div className="flex items-center gap-1.5 border-b border-border bg-muted/50 px-4 py-3">
              ...
              <div className="ml-4 flex h-7 flex-1 max-w-sm items-center gap-2 rounded-lg border border-border bg-background px-3 text-xs text-muted-foreground">
                <Command variant="Bulk" size={13} /> {t("searchOrCommand")}
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-[220px_1fr]">
              ...
              <div className="hidden border-r border-border bg-[var(--paper-2,var(--secondary))] p-3 md:block">
                ...
                <div className="mt-4 space-y-0.5">
                  {[
                    { Icon: Chart2, label: t("navDashboard"), active: true },
                    { Icon: DocumentText, label: t("navDocs") },
                    { Icon: TaskSquare, label: t("navTasks") },
                    { Icon: Calendar, label: t("navCalendar") },
                    { Icon: Data2, label: t("navDatabases") },
                  ].map((n) => (
                ...
                <div className="mt-5 px-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">{t("private")}</div>
                <div className="mt-1.5 space-y-0.5 text-sm text-muted-foreground">
                  {[t("roadmap"), t("okrsQ3"), t("meetingNotes")].map((d) => (
              ...
              <div className="bg-card p-6 md:p-9">
                <div className="mx-auto max-w-2xl">
                  <div className="flex items-center gap-2 text-xs text-muted-foreground"><Hashtag variant="Bulk" size={13} /> {t("productBreadcrumb")} / {t("weeklyPlanHeading")}</div>
                  <h3 className="mt-3 text-3xl font-extrabold tracking-tight">{t("weeklyPlanHeading")}</h3>
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    <span className="tx-pill tx-status-progress"><span className="tx-dot" /> {t("inProgress")}</span>
                    <span className="tx-pill tx-prio-high"><span className="tx-dot" /> {t("high")}</span>
                    <span className="tx-pill" style={{ background: "color-mix(in oklch, #E14B3D 12%, var(--card))", color: "color-mix(in oklch, #E14B3D 80%, var(--foreground))" }}>{t("focusHashtag")}</span>
                  </div>
                    ...
                    {[true, true, false].map((done, i) => (
                      <div key={i} className="flex items-center gap-3 rounded-lg border border-border bg-background px-3 py-2.5 text-sm">
                        ...
                        <span className={done ? "text-muted-foreground line-through" : ""}>
                          {[t("draftProductSpec"), t("syncDesign"), t("shipDashboard")][i]}
                        </span>
                      </div>
                    ))}
                  </div>
      ...
      <section id="features" className="mx-auto max-w-6xl px-5 py-16 sm:px-6 md:py-24">
        <div className="max-w-2xl">
          <span className="text-sm font-semibold uppercase tracking-[0.14em] text-primary">{t("everythingConnected")}</span>
      ...
      <section id="workspace" className="border-y border-border bg-card">
        <div className="mx-auto grid max-w-6xl items-center gap-12 px-5 py-16 sm:px-6 md:grid-cols-2 md:py-24">
          <div>
            <span className="text-sm font-semibold uppercase tracking-[0.14em] text-primary">{t("builtToLast")}</span>
            <h2 className="mt-3 font-display text-4xl font-bold tracking-tight">{t("durableWorkspace")}</h2>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { Icon: DocumentText, label: t("navDocs"), tint: "#E14B3D", lines: 3 },
              { Icon: Chart2, label: t("navFeatures"), tint: "#2f7ea6", lines: 0, chart: true },
              { Icon: TaskSquare, label: t("navTasks"), tint: "#d98324", lines: 0, pills: true },
              { Icon: Calendar, label: t("navCalendar"), tint: "#1f9d76", lines: 2 },
            ].map((c) => (
              <div key={c.label} className="tx-card tx-card-hover p-4">
                ...
                {c.chart ? (
                  ...
                  <div className="mt-4 flex flex-wrap gap-1.5">
                    <span className="tx-pill tx-status-done"><span className="tx-dot" /> {t("done")}</span>
                    <span className="tx-pill tx-status-progress"><span className="tx-dot" /> {t("doing")}</span>
                  </div>
                ) : (
                  <div className="mt-4 space-y-2">
      ...
      <section id="connected" className="mx-auto max-w-6xl px-5 py-16 sm:px-6 md:py-24">
        <div className="grid items-center gap-10 md:grid-cols-2">
          ...
          <div className="grid grid-cols-3 gap-2.5">
            {[
              { n: t("fiveInOne"), l: t("toolsUnified") },
              { n: t("realTime"), l: t("collaboration") },
              { n: t("infiniteNested"), l: t("nestedPages") },
            ].map((s) => (
