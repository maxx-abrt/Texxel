...
const VIEWS = [
  { key: "table", labelKey: "table", icon: RowVertical },
  { key: "gallery", labelKey: "gallery", icon: Element3 },
  { key: "kanban", labelKey: "kanban", icon: Kanban },
  { key: "calendar", labelKey: "calendar", icon: CalIcon },
];
...
export function DatabaseView({ databaseId }: { databaseId: Id<"flux_databases"> }) {
  ...
  if (db === null) {
    return <PageContainer><EmptyState icon={Data2} title={t("notFound")} action={<button onClick={() => router.push("/app/databases")} className={btnOutline}><ArrowLeft2 variant="Bulk" size={16} /> {t("backToList")}</button>} /></PageContainer>;
  }
  ...
  const onImportFile = async (file?: File) => {
    ...
    if (parsed.length < 1) return toast.error(t("emptyCsv"));
    ...
    const headerColIds: string[] = headers.map((hname) => {
      ...
      cols.push({ id, name: hname.trim() || t("columnName"), type: "text" });
      return id;
    });
    ...
    toast.success(t("importedRows", { count: n }));
  };
    ...
    <PageContainer className="max-w-[1280px]">
        ...
        <div className="flex items-center rounded-full border border-border bg-card p-0.5" data-testid="database-view-switch">
          {VIEWS.map((v) => (
            <button key={v.key} onClick={() => setView(v.key)} data-testid={`db-view-${v.key}`} className={cn("flex h-8 items-center gap-1.5 rounded-full px-3 text-sm", view === v.key ? "bg-muted font-medium" : "text-muted-foreground")}>
              <v.icon variant="Bulk" size={15} /> {t(`view.${v.labelKey}`)}
            </button>
          ))}
        </div>
        <div className="flex items-center gap-2">
          {view === "kanban" && selectCols.length > 0 && (
            <Select value={groupBy} onValueChange={(v) => setConfig({ groupBy: v })}>
              <SelectTrigger className="h-8 w-40 text-xs" data-testid="kanban-groupby"><SelectValue placeholder={t("groupBy")} /></SelectTrigger>
              <SelectContent>{selectCols.map((c) => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent>
            </Select>
          )}
          {view === "calendar" && dateCols.length > 0 && (
            <Select value={dateField} onValueChange={(v) => setConfig({ dateField: v })}>
              <SelectTrigger className="h-8 w-40 text-xs" data-testid="calendar-datefield"><SelectValue placeholder={t("dateField")} /></SelectTrigger>
              <SelectContent>{dateCols.map((c) => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent>
            </Select>
          )}
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild><button className={cn(btnOutline, "h-8 text-xs")} data-testid="db-data-menu"><DocumentDownload variant="Bulk" size={15} /> {t("dataMenu")}</button></DropdownMenuTrigger>
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => fileRef.current?.click()} className="gap-2" data-testid="db-import-csv"><DocumentUpload variant="Bulk" size={15} /> {t("importCsv")}</DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={exportCSV} className="gap-2" data-testid="db-export-csv"><DocumentDownload variant="Bulk" size={15} /> {t("exportCsv")}</DropdownMenuItem>
              <DropdownMenuItem onClick={exportJSON} className="gap-2" data-testid="db-export-json"><DocumentDownload variant="Bulk" size={15} /> {t("exportJson")}</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      </div>
        ...
        <div className="overflow-x-auto rounded-2xl border border-border bg-card">
          <table className="w-full border-collapse text-sm" data-testid="database-table">
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      {columns.length > 1 && (
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end"><DropdownMenuItem onClick={() => removeColumn(c.id)} className="text-destructive">{t("deleteColumn")}</DropdownMenuItem></DropdownMenuContent>
                        </DropdownMenu>
                      )}
                    </div>
                  </th>
            ...
            <tbody>
              {(rows ?? []).map((row: any) => {
                ...
                return (
                  <tr key={row._id} className="group border-b border-border last:border-0 hover:bg-muted/30" data-testid="database-row">
                    {columns.map((c) => (
                      <td key={c.id} className="border-r border-border px-1 py-0.5 last:border-0"><Cell type={c.type} options={c.options} defaultValue={cells[c.id]} onChange={(v) => saveCell(row, c.id, v)} t={t} /></td>
                    ))}
              ...
              <tr><td colSpan={columns.length + 1} className="px-3 py-2"><button onClick={() => addRow({ databaseId })} className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground" data-testid="add-row-btn"><Add variant="Bulk" size={16} /> {t("addRow")}</button></td></tr>
            </tbody>
          </table>
        </div>
      )}
      ...
      {view === "gallery" && (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" data-testid="db-gallery">
          {(rows ?? []).map((row: any) => {
            ...
            return (
              <div key={row._id} className="group overflow-hidden rounded-2xl border border-border bg-card transition hover:shadow-md" data-testid="db-gallery-card">
                ...
                <div className="p-3">
                  <p className="truncate font-semibold">{cells[titleCol!] || t("untitled")}</p>
          ...
          <button onClick={() => addRow({ databaseId })} className="flex min-h-[140px] items-center justify-center gap-1.5 rounded-2xl border border-dashed border-border text-sm text-muted-foreground hover:bg-muted/30" data-testid="gallery-add-row"><Add variant="Bulk" size={16} /> {t("new")}</button>
        </div>
      )}

      {view === "kanban" && (
        groupBy ? (
          <KanbanDb columns={columns} rows={rows ?? []} groupBy={groupBy} titleCol={titleCol!} onSetGroup={(row: any, val: string) => setCellNow(row, groupBy, val)} onAdd={(val: string) => addRow({ databaseId }).then((rid: any) => updateRow({ rowId: rid, cells: JSON.stringify({ [groupBy]: val }) }))} t={t} />
        ) : <EmptyState icon={Kanban} title={t("noSelectColumn")} description={t("noSelectColumnDesc")} />
      )}

      {view === "calendar" && (
        dateField ? (
          <CalendarDb rows={rows ?? []} dateField={dateField} titleCol={titleCol!} t={t} locale={locale} />
        ) : <EmptyState icon={CalIcon} title={t("noDateColumn")} description={t("noDateColumnDesc")} />
      )}

      <AddColumnDialog open={colDialog} onOpenChange={setColDialog} onAdd={async (n: string, type: string, o?: string[]) => { await addColumn(n, type, o); setColDialog(false); }} t={t} />
    </PageContainer>
  );
}

function KanbanDb({ columns, rows, groupBy, titleCol, onSetGroup, onAdd, t }: any) {
          ...
          <div className="space-y-2">
            {(grouped[opt] ?? []).map((r: any) => { const cells = JSON.parse(r.cells || "{}"); return (
              <div key={r._id} className="rounded-xl border border-border bg-card p-3 shadow-sm" data-testid="db-kanban-card">
                <p className="text-sm font-medium">{cells[titleCol] || t("untitled")}</p>
                <Select defaultValue={opt === "—" ? "" : opt} onValueChange={(v) => onSetGroup(r, v)}>
                  <SelectTrigger className="mt-2 h-7 text-xs"><SelectValue placeholder={t("set")} /></SelectTrigger>
                  <SelectContent>{(col?.options ?? []).map((o: string) => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent>
                </Select>
              </div>
            ); })}
            {opt !== "—" && <button onClick={() => onAdd(opt)} className="flex w-full items-center justify-center gap-1 rounded-xl border border-dashed border-border py-2 text-xs text-muted-foreground hover:bg-background"><Add variant="Bulk" size={13} /> {t("add")}</button>}
          </div>
        </div>
      ))}
    </div>
...
function CalendarDb({ rows, dateField, titleCol, t, locale }: any) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-border bg-card" data-testid="db-calendar">
      <div className="flex items-center justify-between border-b border-border px-3 py-2">
        <span className="font-semibold">{cursor.toLocaleDateString(locale, { month: "long", year: "numeric" })}</span>
        <div className="flex gap-1">
          ...
          <button onClick={() => setCursor(new Date())} className="rounded-md px-2 py-1 text-sm hover:bg-muted">{t("calendar.today")}</button>
          <button onClick={() => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() + 1, 1))} className="rounded-md px-2 py-1 text-sm hover:bg-muted">›</button>
        </div>
      </div>
      <div className="grid grid-cols-7 border-b border-border bg-muted/40">{[t("calendar.mon"),t("calendar.tue"),t("calendar.wed"),t("calendar.thu"),t("calendar.fri"),t("calendar.sat"),t("calendar.sun")].map((d) => <div key={d} className="px-2 py-1.5 text-center text-xs font-semibold text-muted-foreground">{d}</div>)}</div>
      <div className="grid grid-cols-7">
        {days.map((day, i) => { const k = `${day.getFullYear()}-${day.getMonth()}-${day.getDate()}`; const items = byDay[k] ?? []; const inMonth = day.getMonth() === cursor.getMonth(); return (
          <div key={i} className={cn("min-h-[84px] border-b border-r border-border p-1.5", !inMonth && "bg-muted/20 text-muted-foreground", i % 7 === 6 && "border-r-0")}>
            ...
            <div className="mt-1 space-y-1">{items.slice(0, 3).map((r: any) => { const cells = JSON.parse(r.cells || "{}"); return <span key={r._id} className="block truncate rounded bg-[var(--flux-coral-soft)] px-1.5 py-0.5 text-[11px] font-medium text-primary">{cells[titleCol] || t("untitled")}</span>; })}</div>
          </div>
        ); })}
      </div>
    </div>
...
function Cell({ type, options, defaultValue, onChange, t }: { type: string; options?: string[]; defaultValue: any; onChange: (v: any) => void; t: any }) {
  ...
  if (type === "select") return (
    <Select defaultValue={defaultValue ?? ""} onValueChange={onChange}>
      <SelectTrigger className="h-8 border-0 bg-transparent shadow-none focus:ring-0"><SelectValue placeholder={t("set")} /></SelectTrigger>
...
function AddColumnDialog({ open, onOpenChange, onAdd, t }: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-sm" data-testid="add-column-dialog">
        <DialogHeader><DialogTitle>{t("addColumnTitle")}</DialogTitle><DialogDescription>{t("addColumnDesc")}</DialogDescription></DialogHeader>
        <div className="space-y-3">
          <input autoFocus value={name} onChange={(e) => setName(e.target.value)} placeholder={t("columnName")} className={inputBase} data-testid="column-name-input" />
          <Select value={type} onValueChange={setType}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{TYPES.map((typeKey) => <SelectItem key={typeKey} value={typeKey}>{t(`fieldTypes.${typeKey}`)}</SelectItem>)}</SelectContent></Select>
          {type === "select" && <input value={options} onChange={(e) => setOptions(e.target.value)} placeholder={t("optionsPlaceholder")} className={inputBase} />}
        </div>
        <DialogFooter>
          <button onClick={() => onOpenChange(false)} className={btnOutline}>{t("cancel")}</button>
          <button onClick={() => { if (!name.trim()) return toast.error(t("nameRequired")); onAdd(name.trim(), type, type === "select" ? options.split(",").map((s) => s.trim()).filter(Boolean) : undefined); setName(""); setOptions(""); setType("text"); }} className={btnPrimary} data-testid="column-add-submit">{t("add")}</button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
