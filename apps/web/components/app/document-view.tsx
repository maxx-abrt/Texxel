...
const TAG_COLORS = ["#E14B3D", "#2f7ea6", "#2fbf9b", "#d98324", "#7c5cff", "#e5484d"];
    ...
    <div className="mt-3 flex flex-wrap items-center gap-1.5" data-testid="document-tags">
      {(docTags ?? []).map((t: any) => (
        <span
          ...
          style={{ backgroundColor: `${t.color ?? "#E14B3D"}1f`, color: t.color ?? "#E14B3D" }}
          ...
          {available.length > 0 && (
            <div className="mt-2 max-h-48 space-y-0.5 overflow-y-auto">
              {available.map((t: any) => (
                ...
                >
                  <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: t.color ?? "#E14B3D" }} />
                  {t.name}
                </button>
              ))}
            </div>
