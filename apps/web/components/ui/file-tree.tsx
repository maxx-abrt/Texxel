  ...
  (
    ...
    const setExpandedItems = useCallback(
      (
        ...
        setInternalExpandedItems(updater)
      },
      [isControlled, onExpandedChange, controlledExpandedItems],
    )
            ...
            <AccordionPrimitive.Root
              ...
              onValueChange={(value) => {
                if (value.length > 0) {
                  setExpandedItems((prev) => [
                    ...(prev ?? []),
                    ...value.filter((v) => !prev?.includes(v)),
                  ])
                }
              }}
              dir={dir as Direction}
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ...
      ref={ref}
      className={cn(
        "tx-tree-guide h-full w-px absolute left-1.75 rtl:right-1.75 top-1 bottom-1 rounded-full pointer-events-none",
        className,
      )}
      {...props}
    />
      ...
      >
        ...
        <AccordionPrimitive.Content className="data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down relative overflow-hidden">
          ...
          <AccordionPrimitive.Root
            dir={direction}
            type="multiple"
            className="flex flex-col gap-0.5 py-0.5 ml-5 rtl:mr-5"
            value={expandedItems}
            onValueChange={(value) => {
              setExpandedItems?.((prev) => [
                ...(prev ?? []),
...
export { CollapseButton, File, Folder, Tree, TreeIndicator, type TreeViewElement }
